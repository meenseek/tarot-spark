...
[다음 행동]
[성찰 질문]
전체 흐름은 질문에 대한 카드상 답으로 시작하세요. 답변 초점이나 질문이 상대의 마음을 가리킨다면 첫 두 문장 안에 감정 또는 연애적 관심이 어느 쪽으로 기우는지와 함께 보이는 망설임을 직접 말하고, '알 수 없으니 행동을 보라'는 말로 대신하지 마세요.
...
가능성 A는 '한 가지 해석은 ...일 수 있다는 것입니다.', 가능성 B는 '다른 해석은 ...일 수 있다는 것입니다.'로 시작하세요. 둘은 같은 카드 의미에 근거하지만 결론이 다른 상징적 해석이어야 합니다. 상대가 초점이면 서로 다른 감정적 태도, 관계가 초점이면 서로 다른 상호작용이나 관계 패턴, 독자가 초점이면 서로 다른 감정·기대·선택의 해석을 설명하세요. 입력에 없는 사례를 붙이지 말며, 둘 다 일부 맞거나 모두 틀릴 수 있게 쓰세요.
...
export function buildInstantReadingPrompt(
  ...
  return [
    ...
    "가장 강한 보강·긴장·진행·통합 관계 하나를 골라 질문에 대한 상징적 답을 먼저 제시하세요. 답변 초점이나 공개 질문이 상대의 마음을 가리키면 카드가 뒷받침하는 시선·호감이나 연애적 관심·망설임을 가능성으로 직접 읽으세요.",
